...
module.exports = function makeSchemaStatusable(schema, options) {
  ...
  const GET_METHOD   = 'get' + capitalizeFirstLetter(PREFIX) + 'Status';
  const GET_DETAIL_METHOD = 'get' + capitalizeFirstLetter(PREFIX) + 'StatusDetail';
   ...
   * @return {String}
   */
  schema.methods[GET_METHOD] = function _getStatus() {
    return this.get(_statusProp('value'));
  };

  /**
   * Retrieves the status' detail
   * @return {String}
   */
  schema.methods[GET_DETAIL_METHOD] = function _getStatusDetail() {
    return this.get(_statusProp('detail'));
  };
